...
import {
  ...
  Search, Filter, MoreVertical, Eye, Edit2, CheckCircle, Trash2,
  XCircle, UserCheck, UserX, Home, MapPin, Star,
  DollarSign, TrendingUp, Activity, Settings, ShieldCheck, UserPlus, UserIcon
} from "lucide-react";
...
export default function WebAdminDashboard() {
  ...
  const { data: webAdminsData, error: webAdminsError, mutate: mutateWebAdmins } = useSWR<User[]>(
    activeTab === 'overview' || activeTab === 'manage' ? '/api/admin/sub-admins' : null,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 5000 }
  );
  ...
  // Stats for overview
  const stats = useMemo(() => {
    const webAdmins = webAdminsData || [];

    return {
      total: webAdmins.length,
      approved: webAdmins.filter(u => u.isApproved).length,
      recent: webAdmins.slice(0, 5),
    };
  }, [webAdminsData]);
  ...
  // Web admins filtered & paginated for manage tab
  const filteredWebAdmins = useMemo(() => {
    const data = webAdminsData || [];
    let list = data.slice();
    ...
    return list;
  }, [webAdminsData, searchQuery, sortField, sortOrder]);
  ...
  const handleUpdatePermissions = async (id: string, permissions: any) => {
    ...
    try {
      ...
      await fetch(`/api/admin/sub-admins/${id}/permissions`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/sub-admins/${targetItem._id}`, { method: 'DELETE' });
      if (res.ok) {
        mutateWebAdmins();
      ...
      console.error('Failed to delete web admin', err);
      alert('Failed to delete web admin');
    } finally {
      setActionLoading(null);
    }
        ...
        {editingPermissions && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              {(() => {
                const webAdmin = (webAdminsData || []).find(s => s._id === editingPermissions);
